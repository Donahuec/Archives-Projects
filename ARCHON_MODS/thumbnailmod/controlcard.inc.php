...
{
  ?>
  <div class='ccardcontent'><span class='ccardlabel'>Extent:</span> <?php echo(preg_replace('/\.(\d)0/', ".$1", $objCollection->getString('Extent'))) . " "; ?><?php echo($objCollection->ExtentUnit);?>
  </div>
  <?php
}
...
echo ("<div id='ccardprintcontact' class='smround'> <a href='?p=collections/controlcard&amp;id=" . $objCollection->ID . "&amp;templateset=print&amp;disabletheme=1'><img src='" .$_ARCHON->PublicInterface->ImagePath . "/printer.png' alt='Printer-friendly' /></a> <a href='?p=collections/controlcard&amp;id=" .$objCollection->ID . "&amp;templateset=print&amp;disabletheme=1'>Printer-friendly</a> | <a href='?p=core/contact&amp;f=email&amp;referer= ". urlencode($_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']). "'><img src='" . $_ARCHON->PublicInterface->ImagePath . "/email.png' alt='Email Us' />". " </a><a href='?p=core/contact&amp;f=email&amp;referer= ". urlencode($_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']). "'>Email Us</a></div>");
...
if($objCollection->Scope || !empty($objCollection->Content) || ($objCollection->DigitalContent || $containsImages) || !empty($objCollection->OtherURL))
{
  ...
  <div id="ccardscope" class="mdround">
    ...
    if($objCollection->DigitalContent || $containsImages)
    {
      ...
      <div class='ccardcontent'><span class='ccardlabel'><a href='#' onclick="toggleDisplay('digitalcontent'); return false;"><img id='digitalcontentImage' src='<?php echo($_ARCHON->PublicInterface->ImagePath); ?>/plus.gif' alt='expand icon' /> On-line Images/Records</a></span><br/>
        <div class='ccardshowlist' style="display: none;" id="digitalcontentResults">
          ...
          // VTNM (Video Thumbnail Mod)
          if($containsVideos)
          {
            echo("<span class='bold'><a href='index.php?p=digitallibrary/thumbnails&amp;collectionid={$objCollection->ID}&amp;media=video'>Videos</a></span> (browse thumbnails)<br/>\n\n");
          }
          if($containsAudio)
          {
            echo("<span class='bold'><a href='index.php?p=digitallibrary/thumbnails&amp;collectionid={$objCollection->ID}&amp;media=audio'>Audio</a></span> (browse thumbnails)<br/>\n\n");
          }
          // End VTNM
          if($objCollection->DigitalContent)
          {
            echo("<br/><span class='bold'>Documents and Files:</span></br>");
            foreach($objCollection->DigitalContent as $DigitalContent)
            {
              $objDigitalContent = New DigitalContent($DigitalContent->ID);
              $objDigitalContent -> dbLoad();
              echo("<a href='..?p=digitallibrary/digitalcontent&id=".$objDigitalContent->ID."'>".$objDigitalContent->Title);
              if($objDigitalContent->Date)
              {
                echo(", ".$objDigitalContent->Date);
              }
              echo("</a></br>");
            }
    ...
    {
      echo("<img src='noscan.png' height=50>");

      echo("<br/>No scanned images or documents are available for this collection.");
    }
    ...
    {
      ?>
      <div class='ccardcontent'><span class='ccardlabel'><a href='?p=collections/findingaid&amp;id=<?php echo($objCollection->ID); ?>&amp;q=<?php $ID=key($objCollection->Content); echo("$_ARCHON->QueryStringURL&amp;rootcontentid=$ID")?>'>Detailed Description</a></span><br/>
        <?php
