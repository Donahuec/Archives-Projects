 ...
 * @subpackage AdminUI
 * @author Chris Rishel
 * Last updated by Caleb Braun 7/25/16
 */
...
function digitallibrary_ui_initialize()
{
   ...
   elseif($_REQUEST['f'] == 'Update All')
   {
      echo "hi";
      require_once 'updatecontentfiles.php';
   }
   else
   {
...
function digitallibrary_ui_exec()
{
   ...
   if($_REQUEST['f'] == 'store')
   {
      foreach($arrIDs as &$ID)
      {
         ...
         {
             /* ~~~ Digital Library Mod ~~~ */
             // Get all of the files actually associated with the digital content item
            ...
            foreach($_REQUEST['files'] as $FileID => $array)
            {
               ...
               else
               {
                  ...
                  else
                  {
                      ...
                      unset($newFileArr[$array[directlink]]);
                      /* ~~~ End Mod ~~~ */
                      $objFile->dbStore();
                      $aggregateError .= $_ARCHON->Error;
            ...
            foreach ($errors as $e) {
                $_ARCHON->declareError(basename($e) . " has an unrecognized extension and will not be uploaded.");
            }
            /* ~~~ End Mod ~~~ */
