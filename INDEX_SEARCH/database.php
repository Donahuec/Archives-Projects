...
//can now access and do stuff with it here!!!
{
   ...
   //getting the data that has been passed to the url, this will tell us which function we are using (item, collection, all)
   //puts it in $IndexUtility
   list($IndexUtility) = explode('/', $_REQUEST['indexutility']);

   //here I am just messing around with how to put things into the dialog. you can't easily just insert things
   //insertInformation--> inserts a text row
   $dialogSection->insertRow('name')->insertInformation('Name', 'Index Search');
   $dialogSection->insertRow('choice')->insertInformation('choice', $IndexUtility);

   //Inserts a select box
    $dialogSection->insertRow('Select')->insertSelect('SelectTest', array('one' => 'One', 'two' => 'Two'));
    
    //I believe there is also an insertHTML, but I am unsure
...
<script type="text/javascript">
   ...
   $indexOpts = array ("all" => 'All', "coll" => 'Collection', "item" => 'Item');
   //row is Index search, select is what goes in that row
   $indexSearch = $generalSection->insertRow('Index search')->insertSelect('indexChoices', $indexOpts, array(), NULL, 70);
